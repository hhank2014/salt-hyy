...
nginx_compile:
  cmd.run:
    ...
    - name: ./configure --user=www --group=www --prefix=/usr/local/nginx --with-pcre --with-http_stub_status_module --with-http_ssl_module --add-module=/usr/local/src/ngx_cache_purge-2.3 --add-module=/usr/local/src/nginx-http-concat && make && make install
    - requires:
      - pkg: nginx_pkg
      - cmd: nginx_source
